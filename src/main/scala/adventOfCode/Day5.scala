...
object Day5 extends App {
  ...
  val chart = createChart(Map.empty[Point, Int], lines)
  println(chart.count{case (_, count) => count >= 2 })

  @tailrec
  def createChart(map: Map[Point, Int], lines: List[Line]): Map[Point, Int] = {
    lines match {
      case Nil => map
      case line :: lines =>
  ...
  case class Line(xStart: Int, yStart: Int, xEnd: Int, yEnd: Int) {
    ...
    def getIntermediatePoints: List[Point] = {
      if (this.isDiagonal) List.empty[Point] else {
